...
- **Storage**: `leads/` and `applications/` are symlinks to `/Volumes/Verbatim-Vi560-Media/Development/employment-jobs/{leads,applications}` — the real data lives outside the repo entirely, not just gitignored. This keeps personal comp/PII data out of the repo tree (archiving/zipping the repo folder no longer sweeps it up) and gives every git worktree the same physical copy (the symlinks are tracked, so `git worktree add` recreates them automatically — no more per-worktree divergence of leads/applications data). Skills read/write the relative `leads/...` `applications/...` paths as before; nothing in skill code changed. The absolute path is machine-specific — if this repo is ever cloned to another machine, recreate `employment-jobs/` at the same path or re-point the symlinks.
