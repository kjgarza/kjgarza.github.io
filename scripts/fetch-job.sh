...
# Returns cached qurl content for URL if available, empty string otherwise
qurl_get() {
  qurl get "$1" 2>/dev/null || true
}

# Indexes content in qurl after a successful fetch (best-effort, never fails)
qurl_add() {
  echo "$2" | qurl add "$1" 2>/dev/null || true
}
...
if echo "$URL" | grep -qE 'greenhouse\.io/([^/]+)/jobs/([0-9]+)'; then
  CACHED=$(qurl_get "$URL")
  if [[ -n "$CACHED" ]]; then
    POSTED_DATE=$(extract_date_from_text "$CACHED"); [[ -z "$POSTED_DATE" ]] && POSTED_DATE="unknown"
    emit_json "$URL" "$POSTED_DATE" "open" "$CACHED"
    exit 0
  fi
...
if echo "$URL" | grep -qE 'jobs\.lever\.co/([^/]+)/([a-f0-9-]+)'; then
  ...
  if [[ -n "$CACHED" ]]; then
    POSTED_DATE=$(extract_date_from_text "$CACHED"); [[ -z "$POSTED_DATE" ]] && POSTED_DATE="unknown"
    STATUS=$(detect_closed "$CACHED")
    emit_json "$URL" "$POSTED_DATE" "$STATUS" "$CACHED"
    exit 0
  fi
